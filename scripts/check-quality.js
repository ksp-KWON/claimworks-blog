...
function processPost(filePath) {
  ...
  body = body.replace(
    /(?:^|\r?\n)(?:[ \t]*>?[ \t]*#*[ \t]*💡?[ \t]*)?(?:손해사정사\s*(?:실무\s*조언|실무조언|조언)|^[ \t]*실무\s*TIP)[ \t]*\r?\n[ \t]*>/gm,
    '\n> ### 💡 보상스쿨 실무 TIP\n>'
  );
  ...
  // ── [7. 중복 관련 글 추천 헤더 및 본문 단독 링크 목록 삭제] ─
  body = body.replace(/##\s*🔗?\s*함께\s*읽으면\s*(?:도움이\s*되는|도움되는|좋은)\s*보상\s*(?:칼럼|글)[\s\S]*?(?=\r?\n\r?\n#|$)/gi, '');
  body = body.replace(/(?:^|\r?\n)\[[^\]\n]+\]\(\/blog\/[^\)\n]+\)[ \t]*(?=\r?\n|$)/g, '');

  // ── [8. H3 소제목 공문서식 번호 제거 및 이스케이프된 H2 헤딩 자동 승격] ──
  body = body.replace(/###\s*(?:[가-하]\.|\([가-하]\)|[1-9]\.|\([1-9]\)|[1-9]\))\s*/g, '### ');
  body = body.replace(/(?:^|\r?\n)([1-9])\\\.\s+([^\n]+)/g, '\n\n## $1. $2');

  // ── [8-1. 파손된 핵심 요약 볼드 기호 교정] ─────────────────────────────
  body = body.replace(/(>\s*-\s*)([^\n*:]+?)\*\*\s*:/g, '$1**$2** :');

  // ── [8-2. 시그니처 박스 표준화 (보상스쿨 피드백 & 실무 인사이트)] ──────
  body = body.replace(
  ...
  // ── [9. 마크다운 표(Table) 구분선 및 행 오타 자동 교정] ─────────────────
