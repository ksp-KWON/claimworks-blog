...
const CALCULATORS: CalculatorItem[] = [
  {
    ...
    badgeBg: 'text-blue-600 dark:text-blue-400 border-blue-200/80 dark:border-blue-800/80',
  ...
  },
  {
    ...
    features: ['1~5세대 약관 세대별 공제율', '의원/병원/종합병원별 차등 공제', '3대 비급여 특약 정밀 분리'],
    icon: 'hospital',
    ...
    badgeBg: 'text-emerald-600 dark:text-emerald-400 border-emerald-200/80 dark:border-emerald-800/80',
  ...
  },
  {
    ...
    features: ['대법원 위자료 산정 기준식', '호프만 복리할인 수치 연산', '시중노임단가 및 개호비 산정'],
    icon: 'scale',
    ...
    badgeBg: 'text-rose-600 dark:text-rose-400 border-rose-200/80 dark:border-rose-800/80',
...
export default function CalculatorIndex() {
  return (
    <div className="w-full space-y-6">
      ...
      {/* 1. 상단 메인 헤더 배너 (페이지 안내 배너 룩으로 완전 차별화) */}
      <PremiumCard 
        borderColor="blue" 
        hoverEffect={false} 
      ...
      {/* 2. 3대 계산기 인터랙티브 액션 카드 (선택 카드 룩으로 고도화) */}
        ...
        {CALCULATORS.map((calc) => (
          <Link key={calc.id} href={calc.href} className="group block outline-none">
            <PremiumCard 
              ...
              hoverEffect={true} 
              className="!p-5 sm:!p-7 space-y-5 overflow-hidden transition-all duration-200 group-hover:scale-[1.004]"
            >
              {/* 상단 풀블리드 테마 헤더 바 */}
              <div className={`-mx-5 -mt-5 sm:-mx-7 sm:-mt-7 px-5 py-3.5 sm:px-7 sm:py-3.5 mb-4 bg-gradient-to-r ${calc.headerGradient} border-b ${calc.headerBorder} flex items-center justify-between`}>
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-mono font-bold bg-white dark:bg-zinc-800 px-2 py-0.5 border ${calc.badgeBg}`}>
                    {calc.step}
                  </span>
                  <span className="text-xs sm:text-[13px] font-extrabold text-gray-800 dark:text-gray-200">
                    {calc.badgeText}
                  </span>
                </div>
                <div className="flex items-center gap-1 text-xs font-bold text-gray-500 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                  <span>바로가기</span>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                <div className="flex items-start gap-4">
                  {/* 테마 아이콘 */}
                  <div className={`p-3.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200/80 dark:border-zinc-800 text-${calc.color === 'green' ? 'emerald' : calc.color === 'red' ? 'rose' : 'blue'}-600 shrink-0 mt-1`}>
                    <AppIcon name={calc.icon} size={28} />
                  </div>

                  {/* 텍스트 설명 */}
                  <div className="space-y-2">
                    <div>
                      <h2 className="text-lg sm:text-xl font-black text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                        {calc.title}
                      </h2>
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {calc.features.map((feat, idx) => (
                        <span 
                          key={idx} 
                          className="px-2 py-0.5 bg-gray-100 dark:bg-zinc-800/80 text-[11px] font-bold text-gray-600 dark:text-zinc-400 border border-gray-200/60 dark:border-zinc-700/60"
                        >
                          · {feat}
                        </span>
                      ))}
                ...
                <div className="shrink-0 pt-2 sm:pt-0 sm:pl-4 border-t sm:border-t-0 sm:border-l border-gray-100 dark:border-zinc-800 flex sm:flex-col items-center justify-end">
                  <div className={`w-full sm:w-auto px-5 py-3.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 group-hover:bg-blue-600 dark:group-hover:bg-blue-500 dark:group-hover:text-white font-extrabold text-xs sm:text-sm text-center transition-all shadow-xs flex items-center justify-center gap-2`}>
