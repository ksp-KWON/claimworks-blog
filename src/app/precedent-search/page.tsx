...
function detectVerdictDirection(prec: Precedent): string {
  ...
  const favorableKeywords = ['원고 승', '피고인 패', '보험금 지급', '지급 의무', '지급하여야', '인정된다', '타당하다', '이유 있다'];
  const unfavorableKeywords = ['기각', '원고 패', '지급 거절', '면책', '해지 적법', '이유 없다', '인정할 수 없다'];
  
  const favorableScore = favorableKeywords.filter(kw => text.includes(kw)).length;
  const unfavorableScore = unfavorableKeywords.filter(kw => text.includes(kw)).length;
  
  if (favorableScore > unfavorableScore) {
    return '\n\n💡 이 판례는 소비자(피보험자)에게 유리한 판결 방향으로, 유사한 상황에서 보험금 청구 시 유력한 방어 논리로 활용할 수 있습니다.';
  }
  if (unfavorableScore > favorableScore) {
    return '\n\n⚠️ 이 판례는 보험사 측 주장이 인용된 사례로, 유사한 분쟁에서 소비자가 주의해야 할 법리 기준을 파악하는 데 활용하시기 바랍니다.';
  }
  return '';
}
...
function getPracticeComment(prec: Precedent): string {
  ...
  return baseComment + verdictNote;
}
                        ...
                        <div className="p-5 sm:p-7 space-y-5">
                          ...
                          <div className="bg-[#fcf8e3]/30 dark:bg-[#fcf8e3]/5 p-4 rounded-none border border-[#faebcc]/50 dark:border-[#faebcc]/10 space-y-2">
                            <div className="flex items-center gap-1.5 text-xs font-black text-[#8a6d3b] dark:text-[#c4a86f]">
                              <span className="text-sm">👨‍🏫</span> 보상스쿨 손해사정사 실무 코멘트
                            </div>
                          ...
                          <div className="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-white/5 flex-col sm:flex-row">
                            ...
                            <a href={getKakaoLink(prec)} target="_blank" className="w-full text-center py-3 bg-[var(--google-blue)] hover:bg-[#174ea6] text-white text-xs font-bold shadow-sm transition-colors flex items-center justify-center gap-1.5 cursor-pointer">
                              💬 무료 보상 검토 신청
                            </a>
                          </div>
                        </div>
