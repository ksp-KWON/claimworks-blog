...
export default function ChatWidget() {
  ...
  const handleSetNickname = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const systemMessage = "상담 접수가 완료되었습니다. 전담 손해사정사가 의학 및 법리 내용을 검토 중이며, 약 3~5분 내로 정확한 답변을 드리겠습니다.";
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* Smart Minimal Status Banner (군더더기 없는 슬림 라이브 칩) */}
            {isNicknameSet && (
                  ...
                  <span className="font-bold text-gray-700 dark:text-gray-300">
                    {!hasAdminReplied ? (
                      <span className="flex items-center gap-1.5">
                        <AppIcon name="zap" size={14} className="text-amber-500" />
                        접수 완료 · 손해사정사 내용 검토 중
                      </span>
                    ) : (
                      <span className="flex items-center gap-1.5">
                        <AppIcon name="chat" size={14} className="text-emerald-500" />
                        손해사정사 답변이 도착했습니다
                      </span>
                    )}
                  </span>
            ...
            {!isNicknameSet ? (
              <div className="flex-1 flex flex-col items-center justify-center p-6 bg-white dark:bg-[#18181b]">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 text-center mb-6 break-keep">
                  손해사정사가 직접 확인 후 신속하게 답변해 드립니다.
                </p>
                
                <form onSubmit={handleSetNickname} className="w-full max-w-[300px] space-y-3">
                  <div>
                    <label className="block text-[11px] font-bold text-gray-600 dark:text-gray-400 mb-1">성함 또는 닉네임</label>
                    <input
                      type="text"
                      value={nicknameInput}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-600 dark:text-gray-400 mb-1">사고 분류</label>
                    <div className="relative">
                      ...
                      >
                        ...
                        <option value="배상책임/산재">일상생활배상/산재·근재</option>
                        <option value="후유장해/사망">후유장해/사망보험금</option>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-600 dark:text-gray-400 mb-1">문의 내용</label>
                    <textarea
                      value={inquiryText}
              ...
              <>
                {/* Messages Area */}
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#f8f9fa]/60 dark:bg-[#121214]">
                  ...
                  {(status === 'checking' || status === 'connecting') && (
                    <div className="flex justify-center py-2">
                      <div className="flex gap-1">
                        {[0, 1, 2].map(i => (
                          <div key={i} className="w-1.5 h-1.5 rounded-full bg-[var(--google-blue)]/60 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                        ))}
                      </div>
                    </div>
                  )}
                ...
                <div className="p-3 bg-white dark:bg-[#18181b] shrink-0 border-t border-gray-100 dark:border-zinc-800">
                  <div className="flex items-center gap-2 bg-gray-100/80 dark:bg-[#202024] rounded-full border border-transparent focus-within:border-[var(--google-blue)] focus-within:bg-white dark:focus-within:bg-[#202024] px-4 py-1.5 transition-all shadow-inner">
                    <textarea
                      ...
                      className="flex-1 bg-transparent text-xs sm:text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 resize-none outline-none leading-relaxed max-h-20 overflow-y-auto disabled:opacity-50 py-1"
                    ...
                    >
                      {isSending ? (
                        ...
                        <AppIcon name="send" size={14} className="ml-0.5" />
                      )}
                    </button>
                  </div>
                </div>
