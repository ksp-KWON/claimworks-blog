            ...
            <>
              ...
              <div className="hidden md:flex flex-col space-y-3 pb-8">
                ...
                <div className="flex px-6 py-3.5 bg-white dark:bg-zinc-800 rounded-xl text-sm font-bold text-gray-500 shadow-sm border border-gray-200 dark:border-zinc-700">
                  <div className="w-40 text-center uppercase tracking-wider">발행일</div>
                  <div className="flex-1 text-left pl-4 uppercase tracking-wider">포스팅 제목</div>
                  <div className="w-40 text-center uppercase tracking-wider">관리</div>
                </div>

                {/* 목록 */}
                {sortedAndFilteredList.map((post) => (
                  <PremiumCard key={post.sha} className="p-0 hover:border-blue-300 transition-colors group">
                    <div className="flex items-center px-6 py-4">
                      <div className="w-40 text-center">
                        <PremiumBadge color="gray" className="font-mono font-medium">
                          {post.date || post.name.replace('.md', '')}
                        </PremiumBadge>
                      </div>
                      <div className="flex-1 text-left pl-4 pr-6 truncate">
                      ...
                      <div className="w-40 flex items-center justify-center gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => onLoadPost(post.name, post.sha)}
                          disabled={isLoading}
                          className="px-3 py-1.5 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs font-bold rounded-lg border border-blue-200 dark:border-blue-800/50 transition-colors flex items-center gap-1"
