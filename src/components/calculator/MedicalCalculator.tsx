...
export default function MedicalCalculator() {
  ...
  const shareResult = () => {
    const text = `보상스쿨 실손의료비 계산결과\n▶ 총 진료비: ${result.totalCost.toLocaleString()}원\n▶ 예상 수령액: ${result.totalPayout.toLocaleString()}원\n\n자세한 내역은 보상스쿨에서 확인해보세요!`;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    if (typeof window !== 'undefined' && (window as any).Kakao && (window as any).Kakao.isInitialized()) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (window as any).Kakao.Share.sendDefault({
        objectType: 'feed',
        content: { title: '보상스쿨 실손의료비 결과', description: `예상 보상금: ${result.totalPayout.toLocaleString()}원`, imageUrl: 'https://claim-works.com/og-image.png', link: { mobileWebUrl: window.location.href, webUrl: window.location.href } },
        buttons: [{ title: '결과 보기', link: { mobileWebUrl: window.location.href, webUrl: window.location.href } }],
      });
    } else if (navigator.share) {
      navigator.share({ title: '보상스쿨 실손의료비 계산결과', text: text, url: window.location.href }).catch(console.error);
    } else {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-white dark:bg-[#202124] rounded-3xl p-6 sm:p-7 shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.2)] border border-gray-100 dark:border-white/5 transition-all">
            ...
            {/* 입원/통원 토글 */}
            <div className="flex bg-[#f8f9fa] dark:bg-[#2d2d2d] p-1.5 rounded-2xl mb-5">
              <button onClick={() => handleChange('treatmentType', 'inpatient')} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${data.treatmentType === 'inpatient' ? 'bg-white dark:bg-[#202124] text-[#34A853] shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400'}`}>입원 치료</button>
              <button onClick={() => handleChange('treatmentType', 'outpatient')} className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${data.treatmentType === 'outpatient' ? 'bg-white dark:bg-[#202124] text-[#34A853] shadow-sm' : 'text-gray-500 hover:text-gray-800 dark:text-gray-400'}`}>통원 (외래)</button>
            </div>
              ...
              <div className="space-y-5 animate-in fade-in slide-in-from-top-2">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => handleChange('outpatientDays', Math.max(1, data.outpatientDays - 1))} className="w-12 h-12 bg-[#f8f9fa] dark:bg-[#2d2d2d] rounded-xl flex items-center justify-center font-bold text-gray-600 hover:bg-gray-200">-</button>
                    <div className="relative flex-1">
                      <input type="number" value={data.outpatientDays || ''} onChange={e => handleChange('outpatientDays', Math.max(1, Number(e.target.value)))} className="w-full text-center bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent focus:border-[#34A853] rounded-xl h-12 font-black text-lg focus:ring-0" />
                      <span className="absolute right-4 top-3.5 text-sm font-bold text-gray-400">일</span>
                    </div>
                    <button onClick={() => handleChange('outpatientDays', data.outpatientDays + 1)} className="w-12 h-12 bg-[#f8f9fa] dark:bg-[#2d2d2d] rounded-xl flex items-center justify-center font-bold text-gray-600 hover:bg-gray-200">+</button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-2.5">
                    {HOSPITAL_TYPES.map(ht => {
                      ...
                      return (
                        <button key={ht.id} onClick={() => handleChange('clinicType', ht.id)} className={`flex items-center gap-4 p-3.5 rounded-2xl border-2 transition-all duration-300 ${isActive ? 'border-[#34A853] bg-[#e6f4ea] dark:bg-[#34A853]/10 text-[#34A853] dark:text-[#81c995] shadow-sm' : 'border-transparent bg-[#f8f9fa] dark:bg-[#2d2d2d] text-gray-500 hover:bg-gray-50 dark:hover:bg-[#303134]'}`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <input type="text" inputMode="numeric" value={data.coveredCost ? fmt(data.coveredCost) : ''} onChange={e => handleChange('coveredCost', parse(e.target.value))} placeholder="0" className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border border-gray-200 dark:border-white/10 rounded-2xl py-3.5 pl-5 pr-14 text-lg text-[#202124] dark:text-[#e8eaed] font-black focus:ring-2 focus:ring-[#34A853] focus:bg-white dark:focus:bg-[#202124] focus:outline-none transition-all shadow-inner" />
              ...
              <div>
                ...
                <div className="relative">
                  <input type="text" inputMode="numeric" value={data.nonCoveredCost ? fmt(data.nonCoveredCost) : ''} onChange={e => handleChange('nonCoveredCost', parse(e.target.value))} placeholder="0" className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border border-gray-200 dark:border-white/10 rounded-2xl py-3.5 pl-5 pr-14 text-lg text-[#202124] dark:text-[#e8eaed] font-black focus:ring-2 focus:ring-[#34A853] focus:bg-white dark:focus:bg-[#202124] focus:outline-none transition-all shadow-inner" />
                  <span className="absolute right-5 top-[14px] text-sm text-gray-400 font-bold">원</span>
                </div>
              </div>
            </div>
          ...
          {(data.generation === 3 || data.generation === 4) && (
            <div className="bg-[#fce8e6]/30 dark:bg-[#d93025]/5 rounded-3xl p-6 sm:p-7 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-red-100 dark:border-red-900/20 transition-all animate-in fade-in slide-in-from-top-4">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center py-2.5 border-b border-gray-100 dark:border-white/5">
                  <div>
                    ...
                    <span className="text-[11px] text-gray-400 bg-gray-50 dark:bg-white/5 px-2 py-0.5 rounded-md">항목별 최소공제 및 비율공제 합산</span>
                  </div>
