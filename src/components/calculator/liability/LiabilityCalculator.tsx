      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="bg-white dark:bg-[#202124] rounded-3xl p-6 sm:p-7 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-gray-100 dark:border-white/5 transition-all">
            ...
            <div className="space-y-3">
              {[
                { key: 'hasInjury', emoji: '🩹', title: '부상 (상해)', sub: '휴업손해 및 위자료', activeClass: 'border-[#EF6C00] bg-[#FFF3E0] dark:bg-[#EF6C00]/15', textActive: 'text-[#E65100] dark:text-[#FFCC80]' },
                { key: 'hasDisability', emoji: '🩼', title: '후유장해', sub: '미래 일실수입 (상실수익)', activeClass: 'border-[#EF6C00] bg-[#FFF3E0] dark:bg-[#EF6C00]/15', textActive: 'text-[#E65100] dark:text-[#FFCC80]' },
                { key: 'hasDeath', emoji: '🕊️', title: '사망', sub: '생계비 공제 일실수입 및 장례비', activeClass: 'border-[#EF6C00] bg-[#FFF3E0] dark:bg-[#EF6C00]/15', textActive: 'text-[#E65100] dark:text-[#FFCC80]' },
                { key: 'hasCare', emoji: '👨‍🦽', title: '개호 (간병)', sub: '중증장해로 인한 개호비', activeClass: 'border-[#EF6C00] bg-[#FFF3E0] dark:bg-[#EF6C00]/15', textActive: 'text-[#E65100] dark:text-[#FFCC80]' },
              ].map(item => {
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="relative mb-2">
                  <input type="number" value={data.ageAtAccident || ''} onChange={e => handleChange('ageAtAccident', Number(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-3 pl-4 pr-12 text-[15px] font-black focus:ring-2 focus:ring-[#EF6C00] focus:outline-none transition-all" />
              ...
              <div>
                ...
                <div className="relative mb-2">
                  <input type="text" inputMode="numeric" value={data.income ? fmt(data.income) : ''} onChange={e => handleChange('income', parse(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-3 pl-4 pr-12 text-[15px] font-black focus:ring-2 focus:ring-[#EF6C00] focus:outline-none transition-all" />
                  <span className="absolute right-4 top-3.5 text-[13px] text-gray-400 font-bold">원</span>
                </div>
                <button onClick={() => handleChange('income', 3441360)} className="w-full py-2 bg-[#FFF3E0] dark:bg-[#EF6C00]/15 text-[#E65100] dark:text-[#FFCC80] text-[12px] font-bold rounded-xl hover:bg-[#FFE0B2] transition-all">📊 보통인부 시중노임단가 자동 적용 (3,441,360원)</button>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-2">본인 과실 비율</label>
                <div className="relative mb-2">
                  <input type="number" min="0" max="100" value={data.faultRatio === 0 ? '0' : (data.faultRatio || '')} onChange={e => handleChange('faultRatio', Number(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-3 pl-4 pr-12 text-[15px] font-black focus:ring-2 focus:ring-[#EF6C00] focus:outline-none transition-all" />
              ...
              <div className="space-y-6">
                {data.hasInjury && !data.hasDeath && (
                  <div className="space-y-4 pb-4 border-b border-gray-100 dark:border-white/10 last:border-0">
                    <h4 className="text-[12px] font-black text-[#E65100] flex items-center gap-1.5"><span className="text-lg">🩹</span> 부상 치료 상세</h4>
                    <div>
                      <label className="block text-[11px] font-bold text-gray-500 mb-1">입원 일수</label>
                      <input type="number" value={data.hospitalDays === 0 ? '0' : (data.hospitalDays || '')} onChange={e => handleChange('hospitalDays', Number(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-2.5 px-3 text-[14px] font-bold focus:ring-2 focus:ring-[#EF6C00]" />
                    </div>
                  </div>
                )}
                ...
                {data.hasDisability && !data.hasDeath && (
                  <div className="space-y-4 pb-4 border-b border-gray-100 dark:border-white/10 last:border-0">
                    <h4 className="text-[12px] font-black text-[#E65100] flex items-center gap-1.5"><span className="text-lg">🩼</span> 후유장해 상세</h4>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 mb-1">노동능력상실률 (%)</label>
                        <input type="number" value={data.disabilityRate === 0 ? '0' : (data.disabilityRate || '')} onChange={e => handleChange('disabilityRate', Number(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-2.5 px-3 text-[14px] font-bold focus:ring-2 focus:ring-[#EF6C00]" />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 mb-1">장해 기간 (0=영구)</label>
                        <input type="number" value={data.disabilityYears === 0 ? '0' : (data.disabilityYears || '')} onChange={e => handleChange('disabilityYears', Number(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-2.5 px-3 text-[14px] font-bold focus:ring-2 focus:ring-[#EF6C00]" />
                      </div>
                    </div>
                  </div>
                )}
                ...
                {data.hasCare && (
                  <div className="space-y-4 pb-4 border-b border-gray-100 dark:border-white/10 last:border-0">
                    <h4 className="text-[12px] font-black text-[#E65100] flex items-center gap-1.5"><span className="text-lg">👨‍🦽</span> 개호(간병) 상세</h4>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 mb-1">일일 필요 개호인 (명)</label>
                        <input type="number" step="0.5" value={data.carePersons === 0 ? '0' : (data.carePersons || '')} onChange={e => handleChange('carePersons', Number(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-2.5 px-3 text-[14px] font-bold focus:ring-2 focus:ring-[#EF6C00]" />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold text-gray-500 mb-1">개호 기간 (0=영구)</label>
                        <input type="number" value={data.careYears === 0 ? '0' : (data.careYears || '')} onChange={e => handleChange('careYears', Number(e.target.value))} className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-2.5 px-3 text-[14px] font-bold focus:ring-2 focus:ring-[#EF6C00]" />
                      </div>
                    </div>
                  </div>
                )}
                ...
                {data.hasDeath && (
                  <div className="bg-[#FFF3E0] dark:bg-[#EF6C00]/10 border border-[#FFB74D]/50 rounded-xl p-4 text-center">
                    <span className="text-2xl mb-1 block">🕊️</span>
                    <p className="text-[12px] font-bold text-[#E65100] dark:text-[#FFCC80]">사망 사고 산정 기준 자동 적용</p>
                    <p className="text-[11px] text-[#F57C00] dark:text-[#FFB74D] mt-1">일실수입에서 생계비(1/3) 공제 / 위자료 장해율 100% 적용</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-[#202124] rounded-3xl p-6 sm:p-7 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-gray-100 dark:border-white/5 transition-all">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={field}>
                  ...
                  <div className="relative">
                    <input type="text" inputMode="numeric" value={data[field] ? fmt(data[field] as number) : ''} onChange={e => handleChange(field, parse(e.target.value))} placeholder="0" className="w-full bg-[#f8f9fa] dark:bg-[#2d2d2d] border-transparent rounded-xl py-3 pl-4 pr-12 text-[15px] font-black focus:ring-2 focus:ring-[#EF6C00] focus:outline-none transition-all" />
                    <span className="absolute right-4 top-3.5 text-[13px] text-gray-400 font-bold">원</span>
                  </div>
                </div>
              ))}
              {data.hasDeath && (
                <div className="pt-2">
                  <label className="block text-xs font-bold text-[#E65100] mb-2">장례비</label>
                  <div className="relative">
                    <input type="text" inputMode="numeric" value={data.funeralCost ? fmt(data.funeralCost) : ''} onChange={e => handleChange('funeralCost', parse(e.target.value))} className="w-full bg-[#FFF3E0] dark:bg-[#EF6C00]/10 border-transparent rounded-xl py-3 pl-4 pr-12 text-[15px] font-black text-[#E65100] focus:ring-2 focus:ring-[#EF6C00] focus:outline-none transition-all" />
                    <span className="absolute right-4 top-3.5 text-[13px] text-[#F57C00] font-bold">원</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="lg:col-span-7 lg:sticky lg:top-[100px] flex flex-col gap-5">
          ...
          <div ref={resultRef} className="flex flex-col gap-5">
            ...
            <div className="bg-gradient-to-br from-[#FF8F00] to-[#E65100] dark:from-[#E65100] dark:to-[#FF8F00] rounded-3xl p-8 sm:p-10 text-white shadow-xl shadow-[#EF6C00]/20 relative overflow-hidden transition-all duration-300 hover:scale-[1.01]">
            ...
            <div className="bg-white dark:bg-[#202124] rounded-3xl border border-gray-200 dark:border-white/10 p-7 shadow-sm">
              <h3 className="text-[13px] font-extrabold text-[#202124] dark:text-[#e8eaed] flex items-center gap-2 mb-5">
                <span className="w-1 h-4 bg-[#EF6C00] rounded-full"></span> 세부 보상 내역
              </h3>
              ...
              <div className="space-y-4 text-[13px] font-medium text-gray-600 dark:text-gray-400">
                {result.alimony > 0 && (
                  <div className="flex justify-between items-center py-2.5 border-b border-gray-100 dark:border-white/5">
                    <span>정신적 손해 (위자료) <span className="text-[10px] bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded ml-1">{result.effectiveDisabilityRate}%</span></span>
                ...
                <div className="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-white/5 mt-1">
                  <span className="text-[15px] font-extrabold text-[#EF6C00]">최종 예상 배상액</span>
                  <span className="text-[18px] font-black text-[#EF6C00]">{Math.floor(result.totalAmount).toLocaleString()} 원</span>
                </div>
              </div>
          ...
          <div className="bg-[#FFF3E0]/80 dark:bg-[#EF6C00]/10 rounded-2xl p-4 border border-[#FFB74D]/50 flex gap-3 text-[12px] leading-relaxed text-[#E65100] dark:text-[#FFCC80] font-semibold shadow-sm">
