...
function normalizeMarkdownBody(rawBody, fallbackSummary = '') {
  ...
  // ── [9-2. 괄호 뒤 볼드 + 한글 조사 결합 시 마크다운 파서 파손 원천 방지] ───
  // CommonMark 6.2 사양상 )** 뒤에 한글 조사가 공백 없이 붙으면 볼드가 안 닫히는 현상 근본 해결
  // 예: **용어(English)**과 -> **용어**(English)과 (핵심어만 볼드로 묶고 부연 괄호는 자연스럽게 분리)
  body = body.replace(/\*\*([^*\n()]+)\(([^)\n]+)\)\*\*([가-힣])/g, '**$1**($2)$3');
