...
- **포맷팅 자율성 및 버그 방지**: 정보를 시각화할 때는 표준 마크다운 표(\`|---|---|\`), 인용구(\`>\`), 리스트 기호(\`-\` 또는 \`1. 2. 3.\`) 등을 자유롭게 활용하여 가독성을 극대화하십시오. 리스트 앞뒤로는 빈 줄(Enter)을 꼭 두어 렌더링 오류를 예방하세요. 창의적인 시각화가 필요하다면 특수기호를 활용할 수도 있지만, 가독성을 최우선으로 고려하세요.
