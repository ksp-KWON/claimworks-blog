...
## 11. 고품질 블로그 스타일링 및 가독성 극대화 수칙 (자율적 시각화 및 요약 툴 적극 활용)
- **긴 줄글(Wall of Text) 절대 금지**: 3~4줄 이상 넘어가는 빽빽한 서술형 문단은 독자의 가독성을 심각하게 떨어뜨립니다. 긴 설명이나 나열이 필요한 경우 절대 산문형으로 길게 늘어쓰지 말고, 아래의 '요약 및 시각화 툴'을 자율적이고 적극적으로 활용하여 호흡을 끊어주십시오.
- **AI의 자율적 판단에 따른 시각화 및 구조화**: 밋밋한 텍스트 나열을 지양하고 독자의 가독성과 시인성을 극대화하는 것이 궁극적인 목표입니다.
  1) **마크다운 표(Table)**: 정보의 압축, 요약, 개념 비교가 필요한 맥락에서 자율적으로 판단하여 삽입
  2) **핵심 요약 박스 (인용구 툴 활용)**: 판례의 핵심, 대응 절차, 의학적 소견 요약 등 묵직한 정보 덩어리는 마크다운 인용구(\`>\`)를 활용한 콜아웃/요약 박스로 감싸서 시각적으로 분리하십시오.
  3) **단계별 넘버링 및 리스트**: 절차, 가이드, 체크리스트 등은 단순 서술 대신 \`1단계:\`, \`2단계:\` 등의 넘버링과 불릿포인트 리스트(\`-\`)를 활용하여 구조화하십시오.
  4) **계층적 소제목 (H3, H4)**: 텍스트의 호흡이 길어질 경우 논리 전개에 맞춰 H3(\`###\`), H4(\`####\`) 소제목을 자율적으로 분할하여 가독성 증대
  5) **키워드 컬러 강조**: 독자의 시선을 사로잡아야 할 핵심 쟁점, 팁, 주의사항에 마크다운 색상 태그(\`<blue>\`, \`<red>\`, \`<green>\`, \`<orange>\` 등)와 볼드체(\`**텍스트**\`)를 문맥에 맞게 적재적소에 배치
- **포맷팅 버그 방지**: 리스트 작성 시 기호 없이 시작하면 줄바꿈이 뭉개지거나 폰트 렌더링에 오류(자간/줄간격 어색해짐)가 발생합니다. 반드시 표준 기호(\`-\` 또는 \`1. 2. 3.\`)를 쓰고, 리스트 앞뒤로 한 줄 빈 줄(Enter)을 꼭 두세요. 코드 블록(\`\`\`\`)은 전면 금지하며, 인용구(\`>\`)만 허용합니다.
