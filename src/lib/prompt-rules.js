...
- **[무기 7] E-E-A-T 클로징 맞춤형 다단계 솔루션 & CTA 배너 (Shield Check SVG)**: 결론부 '## 5. 결론 및 보상스쿨의 맞춤형 솔루션' 아래에 '###### ① [해결 솔루션 제목]'과 설명 문단으로 구성하십시오. (시스템이 프리미엄 에메랄드 그린 승소 카드와 상담 신청 배너로 자동 렌더링합니다.)
- **[공통 서식] 마크다운 비교표 (Table) & 인라인 용어사전**: 복잡한 법리/약관은 표준 마크다운 표('| 구분 | 내용 |')로, 전문 용어는 '> **용어명** : 설명' 인라인 인용구로 작성하십시오.

## 3. 문체 규칙, 표준 문단 호흡 및 키워드 강조
- **표준 문단 호흡 규칙 (GFM Paragraph Breathing Rule)**: 하나의 완결된 생각(논리 덩어리)을 하나의 문단으로 묶고, 다음 문단으로 전환 시 **반드시 표준 빈 줄 1개(`\n\n`)로 구분**하십시오. 모바일 및 PC 가독성을 위해 기본 **2~4문장(150~250자 내외)의 호흡을 권장**하며, 5문장 이상 빽빽하게 이어지는 벽돌 문단(Wall of Text)이나 문장 중간 강제 엔터/`<br>`에 의한 텍스트 파편화는 금지합니다.
