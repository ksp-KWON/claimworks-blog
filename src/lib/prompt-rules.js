...
- **키워드 강조 ('**볼드**') 및 5대 톤 접두사 필수 규칙**: 중요한 법리 및 인사이트에 문단당 1~2개 이하로 절제하여 볼드 처리를 하되, **반드시 아래 5대 감정·법리 톤 접두사 중 하나를 볼드 기호 안쪽 첫머리에 붙이십시오**:
  1) **R:** : 위험·경고·면책·부지급·삭감·주의·금지·불리한 함정 (예: **R:절대로 합의서에 서명해서는 안 됩니다**)
  2) **E:** : 지급·보상·합의 성공·승소·전액 수령·권리 인정·방어 성공 (예: **E:손해배상금 전액을 정상 지급받을 수 있습니다**)
  3) **A:** : 핵심 체크포인트·필수 확인 사항·중요 기준·실무 팁 (예: **A:사고 직후 촬영한 정밀 영상 결과지**)
  4) **P:** : 전문 법리·판례 법리·의학적 감정·손해사정 실무 용어 (예: **P:상법 제655조 단서의 인과관계 입증 책임**)
  5) **B:** : 기본 설명·일반 객관적 강조 (예: **B:사고 발생일로부터 3년의 소멸시효**)
  - **선택 원칙**: 단순히 특정 단어에 얽매이지 말고, **문장의 실제 어조(부정문·경고문인지, 긍정 해결책인지)**를 문맥 전체로 판단하여 가장 정확한 접두사를 선택하십시오. (시스템 렌더러가 접두사를 자동으로 인식해 최적의 파스텔톤 배경을 적용하며, 화면에는 접두사 없이 본문만 깨끗하게 표시합니다.)
...
const CONTENT_SCHEMA = {
  ...
  properties: {
    ...
    markdownContent: {
      type: "STRING",
      description: "글로벌 마크다운 헌법 규칙을 100% 준수한 블로그 본문 마크다운 전문 (Frontmatter 제외, 볼드 강조 시 문맥 어조에 따라 **R:경고**, **E:해결**, **A:체크**, **P:법리**, **B:일반** 접두사 필수 적용)"
    }
  },
