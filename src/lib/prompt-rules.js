...
- 본문을 작성하기 전에 반드시 아래 형식의 분석 블록을 출력의 최상단에 작성하십시오:
  ...
  - 주제 핵심 쟁점 : {디스크, 골절, 인대 등 현재 주제에 맞는 고도의 의학적 진단 쟁점 및 법률적 약관 쟁점 상세 분석}
  - 필요한 보상 실무 지식 : {이 주제의 보상 사정에 활용할 실제 평가 기준(예 : 후유장해 판단법, 사고 관여도 산정법, 검사법 등)을 동적으로 도출}
  - 섹션별 분량 경중 계획 :
    * 섹션 1 (공감 서론) : {분량 경중 계획 서술. 예 : 1.5문단으로 간결히 독자 상황 공감}
    * 섹션 2 (보험사 전술) : {분량 경중 계획 서술. 예 : 3.5문단 이상의 최대 상세 분석 및 구체적 근거 예시}
    * 섹션 3 (결과 비교) : {분량 경중 계획 서술. 예 : 2문단과 대비 표로 명확한 보상 차이 대조}
    * 섹션 4 (추가 분쟁) : {분량 경중 계획 서술. 예 : 3문단으로 심화 법리 및 추가 변수 분석}
    * 섹션 5 (상담 유도) : {분량 경중 계획 서술. 예 : 1.5문단으로 강요 없이 설득력 있는 마무리}
  [ANALYSIS_END]
- AI는 위 사전 분석 단계에서 정립한 주제별 세부 지식 포인트와 섹션별 분량 계획을 철저히 엄수하여 본문을 작성해야 합니다. 중요도가 가장 높은 핵심 분석 섹션(섹션 2, 섹션 4)은 전문 의학/법률 지식을 결합해 아주 깊고 방대하게 서술하고, 비교적 가벼운 섹션(섹션 1, 섹션 5)은 콤팩트하게 작성함으로써, 지정된 타겟 모델 물리 스펙의 안전 허용 범위 이내에서 출력 텍스트의 끊김이나 퀄리티 저하(무의미한 사족 반복) 없이 가장 알찬 고밀도의 전문 칼럼을 자율적으로 구성하십시오.
- 단순히 "주의해야 한다"로만 끝내지 말고, **해당 부상/질병에 따른 구체적인 의학적 진단 기준 수치나 척도, 보험사가 자사 자문의를 통해 보험금을 삭감하는 구체적인 실무 수법, 실제 대법원 판례 및 법원 판결의 논리적 배경**을 실무 사례 에피소드와 함께 아주 상세하고 길게 풀어쓰십시오.
...
function getBlogObjective(keywords) {
  return `# Objective
타겟 키워드 [${keywords}]를 기반으로, 아래의 공통 글쓰기 헌법 규칙을 완벽히 만족하며 구글 E-E-A-T 및 YMYL 기준에 부합하도록, 타겟 모델의 물리 한계 스펙 내에서 아주 상세하고 방대한 분량의 초고품질 전문 칼럼을 작성합니다.
...
분량은 전문성을 높이기 위해 타겟 모델의 물리적 출력 한계 내에서 최대한 상세하고 깊이 있게 기술해 주십시오.`;
}
...
function getBlogLengthRulesManual() {
  return `# 분량 확보 및 정보 보완 규칙
- 원문에 기재된 사건 팩트(진단 결과, 합의 상황 등)는 절대 임의로 변형하거나 왜곡하지 마십시오.
- 다만, 타겟 모델의 물리적 출력 허용 한계 안에서 최고 품질의 풍부한 내용을 확보하기 위해 원문에는 생략되어 있는 **의학적/법률적 세부 정보(맥브라이드 장해 진단 기준, 퇴행성 기왕증 삭감 수법, 판례 및 관계 법령, 구체적인 계산 방식 등)**를 당신의 전문 지식을 동원하여 최대한 상세히 덧붙여서 확장 서술하십시오.
...
- 타겟 모델의 물리적 출력 허용 한계 내에서 충분한 분량을 달성하기 위해 **의학적 장해 측정 기준, 보험사 면책 분쟁 전술, 대법원 판례 논리 및 실무 합의금 계산 방식 등**의 유익하고 구체적인 정보를 매우 풍부하게 보완하여 작성하십시오.
...
[ANALYSIS_START]
- 부여된 타겟 모델 물리 스펙 및 권장량 : {{TARGET_MODEL_CAPACITY}}
- 주제 핵심 쟁점 : {판례의 핵심 법리 및 의학적 쟁점 분석}
- 필요한 보상 실무 지식 : {판례를 실무 손해사정에 대입할 방법}
- 섹션별 분량 경중 계획 :
  * 섹션 1 (사건 배경) : {계획}
  * 섹션 2 (법원 판단) : {계획}
  * 섹션 3 (실무 적용) : {계획}
