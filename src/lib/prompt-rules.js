...
- 대신 상황에 맞는 이모지와 볼드체를 결합하여 자연스럽게 강조하십시오.
  - 경고/위험/금지 : **🔴 강조할 키워드**
  - 주의/참고 : **🟠 강조할 키워드**
  - 긍정/해결/안전 : **🟢 강조할 키워드**
  - 핵심 강조 : **🔵 강조할 키워드**
...
당신에게는 마크다운 문법 오류를 일으키지 않는 선에서 **분량, 글의 구조, 섹션(H2, H3) 구분 등에 대한 100% 무한한 자유도**가 주어집니다. 
과거의 딱딱한 템플릿(코어 1~5)은 모두 삭제되었습니다. 오직 '어떻게 해야 고객이 상담을 요청할까?'라는 목표에만 집중하여 가장 매력적이고 방대한 칼럼을 자율적으로 설계하십시오.

단, 마크다운 렌더링 및 SEO를 위해 아래의 **최소한의 가이드라인(운동장)** 안에서 마음껏 뛰어노십시오.

1. **도입부**: 독자의 절박한 상황에 공감하며 시작하고, 반드시 <규칙 3>의 '저자 경험 박스'를 초반에 삽입하십시오.
2. **본문 전개**: [${angle.name}] 관점에 맞춰 H2, H3를 자율적으로 생성하며 텍스트, 표, 인용구 등을 다채롭게 활용하십시오. 
3. **내부 링크 삽입**: 문맥이 자연스럽게 닿는 곳에 아래 연관 글 링크를 자율적으로 배치하십시오. (강제 아님, 필요시 사용)
[기존 글 목록]
${postsCtx}
4. **수임 유도 (콜투액션)**: 본문 후반부에 독자가 스스로 해결하기 어렵다는 점(보험사의 거대함, 의학적/법률적 입증의 어려움)을 각인시키고 자연스럽게 전문가 상담을 유도하는 섹션을 자율 생성하십시오. (자가진단 체크리스트 등을 활용해도 좋습니다)
5. **FAQ (선택사항)**: 구글 노출을 위해 질의응답이 필요하다고 판단되면 반드시 <규칙 6>의 FAQ 마크다운 포맷(## 💡 자주 묻는 질문 (FAQ) -> ### Q: -> A:)을 엄수하여 구성하십시오.
`;
}
