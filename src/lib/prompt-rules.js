...
- **[무기 7] 다이내믹 목차(TOC)** : \`##\` (H2) 헤딩 (우측 플로팅 목차 자동 생성)
- **[무기 8] 전역 보상금 계산기** : (시스템 자동 삽입되므로 본문에 작성 불필요. 클로징 시 언급 유도용)
- **[무기 9] 공문서 체계 자동 렌더러** : 규칙 2번의 \`1., 가., 1), ①\` 등의 계층형 텍스트는 프론트엔드가 자동으로 예쁜 프리미엄 헤딩과 들여쓰기로 렌더링(마사지)해 주므로, AI는 안심하고 규칙 2번의 기호 체계로만 텍스트를 기획할 것.
- **[무기 10] 마크다운 코드 블록(\`\`\`) 사용 금지** : 마크다운 표(Table), 텍스트 강조, 요약 박스, 계산 공식 작성용으로 절대 사용 금지. (표를 코드블록으로 감싸면 텍스트로 렌더링되어 치명적 오류 발생). 요약/개요 박스가 필요하면 인용구(\`> ### 💡 사건 개요\`)나 표를 사용하십시오.
...
4. **마크다운 표(Table)**: 복잡한 정보(비교, 기준 등)를 마크다운 표(\`|---|---|\`)를 사용하여 최소 1개 이상 정리할 것.
5. **자율적 시각화 무기 활용**: 1분 자가진단, 용어 사전, 실무쟁점 팁 박스 등 9대 무기를 문맥에 맞게 자율적이고 유기적으로 활용할 것.
...
- 이때 억지스러운 영업 멘트 대신, **공문서 목차 체계의 특수 강조 기호(①, ②, ③ 등 - 무기 9)**를 활용하여, 방금 서술한 고객의 억울한 상황을 타개할 수 있는 **[보상스쿨의 3단계 맞춤형 전문 솔루션]**을 매력적으로 디자인하십시오.
