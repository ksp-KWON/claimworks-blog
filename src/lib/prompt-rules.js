...
- 🚨 **[치명적 주의사항]** : 형광펜 태그를 열었으면 반드시 동일한 태그로 닫아야 합니다 (예: `<blue>중요내용</blue>`). 태그를 닫지 않으면 전체 문서 레이아웃이 붕괴되므로 렌더링 에러가 절대 발생하지 않도록 각별히 유의하십시오. 보조적인 텍스트 강조는 일반 볼드체(`**강조**`)를 혼용해도 좋습니다.
...
1. **용어설명 박스**: 전문 용어가 나올 경우 인용구(\`>\`) 등을 활용하여 이해하기 쉽게 풀어서 설명.
2. **손해사정사 실무조언 박스**: 단순 정보 전달을 넘어 실무자의 꿀팁을 인용구(\`>\`) 형태로 제공.
