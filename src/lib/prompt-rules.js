...
- **AI의 자율적 판단에 따른 시각화 및 구조화**: 밋밋한 텍스트 나열을 지양하고 독자의 가독성과 시인성을 극대화하는 것이 궁극적인 목표입니다. 기계적인 개수나 형식 강제(하드코딩) 없이, 글의 방대한 내용과 흐름에 따라 AI가 스스로 판단하여 다음의 디자인 요소들을 가장 조화롭고 입체적으로 활용하십시오.
  1) **마크다운 표(Table)**: 정보의 압축, 요약, 개념 비교가 필요한 맥락에서 자율적으로 판단하여 삽입
  2) **단계별 넘버링 및 리스트**: 절차나 가이드 등 명확한 구분이 필요한 곳에서 \`1단계:\`, \`2단계:\` 등의 넘버링과 불릿포인트 리스트(\`-\`)를 유동적으로 활용
  3) **계층적 소제목 (H3, H4)**: 텍스트의 호흡이 길어질 경우 논리 전개에 맞춰 H3(\`###\`), H4(\`####\`) 소제목을 자율적으로 분할하여 가독성 증대
  4) **키워드 컬러 강조**: 독자의 시선을 사로잡아야 할 핵심 쟁점, 팁, 주의사항에 마크다운 색상 태그(\`<blue>\`, \`<red>\`, \`<green>\`, \`<orange>\` 등)와 볼드체(\`**텍스트**\`)를 문맥에 맞게 적재적소에 배치
