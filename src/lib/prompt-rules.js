...
- **1단계 대주제 (H2, `##`) [표준 기승전결 뼈대]**: 
  - 본문 대주제는 기본적으로 **`## 1. [쟁점 및 법리]` ➔ `## 2. [단계별 실무 절차]` ➔ `## 3. [비교 분석 및 반박]` ➔ `## 4. [의학적 요건 및 장해 기준]`** 의 4대 핵심 뼈대와 마지막 **`## 5. 결론 및 보상스쿨의 맞춤형 솔루션`** 체계로 확고하게 전개합니다.
- **2단계 중주제 (H3, `###`) [AI 무한 자유도]**: 
  - 각 H2 챕터 안에서 필요한 만큼 세부 주제를 `### 질병분류코드와 보험사의 면책 논리`, `### 의무기록 감정 및 인과관계 쟁점` 등 W3C & Google SEO 표준(GFM)에 따라 AI가 100% 자율적·유연하게 무제한 구성하십시오. (공문서식 '가.', '나.', '다.' 두문자 없이 깔끔한 서술형 소제목 권장)
- **3단계 하위 목록 구조 (Semantic Lists)**:
  - 순서 없는 나열: `- ` 또는 `* ` (불릿 포인트)
  - 순서 있는 절차: `1. `, `2. `, `3. ` (순서 리스트)
  - 자가진단 체크: `- [ ] ` (인터랙티브 체크박스)
- **반괄호 숫자/한문식 소모 제목 사용 절대 금지**: `1)`, `가)`, `가.`, `(1)`, `①` 등의 기호를 제목(Heading) 자리에 사용하지 마십시오.

## 2. ⚔️ 보상스쿨 7대 핵심 시각화 무기 (자유로운 필수 배치)
정보를 시각화할 때 비전문적이거나 조잡한 텍스트 기호(+, ---, ASCII 아트)를 직접 그리지 마십시오. HTML 태그(<div> 등)는 금지됩니다.
- **[무기 1] 핵심 요약 포인트 박스 & 감성 오프닝**:
  - 포스트 시작 시 반드시 최상단에 `## 💡 핵심 요약` 제목 아래 3개 불릿 포인트(`> - **핵심 키워드** : 구체적 설명`)로 핵심 요약을 먼저 작성합니다.
  - 핵심 요약 직후 독자의 고통에 공감하고 현실적 문제의식을 던지는 따뜻한 '오프닝 서술 문단(도입부 2~3문장)'을 전개합니다.
- **[무기 2] 마크다운 비교표 (Table)**:
  - 복잡한 법리, 약관, 세대별 차이, 보험사 주장 vs 손해사정사 반박 등 구조화된 데이터는 반드시 깨끗한 표준 마크다운 표(`| 구분 | 내용 |`, `|---|---|`)로 1개 이상 정리하십시오.
- **[무기 3] 용어 설명 글로서리 인용구 박스**:
  - 전문 의학/보험 용어가 등장하는 문단 바로 아래에 `> 💡 **용어명** : 설명` 인용구 박스를 자유롭게 삽입하십시오. (예: `> 💡 **외상 기여도** : 손해 및 신체 장해 발생에 있어 신체적 결함(기왕증) 외에 사고라는 외력이 영향을 미친 비율을 비율(%)로 나타낸 것입니다.`)
- **[무기 4] 보상스쿨 실무 팁 / 손해사정사 조언 박스**:
  - 실무 노하우나 주의사항을 강조할 때 `> ### 💡 손해사정사 실무 조언` 또는 `> 💡 **실무 팁** : ...` 형태의 인용구 조언 박스를 본문 적재적소에 자유롭게 배치하십시오.
- **[무기 5] 1분 자가진단 체크리스트**:
  - 1번 본문 챕터 직후 또는 중반부에 `## 📋 1분 자가진단 체크리스트` 제목을 작성하고, 그 아래 `- [ ] ` 형식의 체크박스 4~5개를 배치하십시오.
- **[무기 6] 자주 묻는 질문 (FAQ)**:
  - 본문 후반부에 `## ❓ 자주 묻는 질문 (FAQ)` 제목을 반드시 작성하고, 실무 관점의 핵심 Q&A 3개를 `### Q : [질문]` 과 `**A** : [답변]` 포맷으로 구성하십시오.
- **[무기 7] E-E-A-T 클로징 맞춤형 3단계 솔루션**:
  - 마지막 결론부 `## 5. 결론 및 보상스쿨의 맞춤형 솔루션` 아래에 사안의 복잡도와 성격에 맞추어 **`- **솔루션 1 제목** : 설명 문단`**, **`- **솔루션 2 제목** : 설명 문단`**, **`- **솔루션 3 제목** : 설명 문단`** 형태로 AI가 최적의 솔루션 단계(2단계 이상 자율적 다단계 확장)를 자유롭게 구성하여 전문성과 신뢰도를 극대화하십시오.

## 3. 🎨 문체 규칙 및 컬러 하이라이트
- **키워드 및 인사이트 강조 (`**볼드**`, `<color>`)**: 중요한 법리, 핵심 키워드, 인사이트 문장은 `**강조**` 또는 `<blue>`, `<green>`, `<orange>`, `<red>` 태그를 활용하십시오.
- **콜론(:) 띄어쓰기**: 콜론 앞뒤로 무조건 한 칸씩 공백을 둡니다. (예: `분쟁의 본질 : 약관의 해석`)
- **문장 종결**: 정중한 존댓말로 통일합니다. (~합니다, ~바랍니다)
- **CTA 및 관련글 수동 작성 금지**: "보상스쿨에 문의하세요", "함께 읽으면 좋은 글" 등의 문구나 관련 글 링크 박스를 본문 끝에 임의로 작성하지 마십시오. (하단 시스템 컴포넌트가 자동으로 렌더링함)
`;
...
1. [생각의 사슬 (Chain-of-Thought) 필수]: 본문 작성 전, 원문의 핵심 법리를 네이버 이웃 독자가 공감할 수 있는 따뜻한 스토리텔링으로 전환하는 전략, 모바일 호흡(줄바꿈) 배치 계획, 6대 헌법 무기(핵심요약/오프닝/비교표/인라인사전/자가진단/FAQ/맞춤솔루션) 장착 설계를 thoughtProcess에 연쇄 사고로 먼저 심층 기술하십시오.
2. 네이버 D.I.A.+ 문체 헌법:
...
3. 6대 헌법 무기를 100% 완벽하게 장착하십시오.`;
  } else if (mode === 'naver-expand') {
    modeInstruction = `[네이버 블로그 D.I.A.+ 신규 확장 창작 특명]
1. [생각의 사슬 (Chain-of-Thought) 필수]: 본문 작성 전, 실제 피해자가 겪는 현실적 고통과 검색 의도 분석, 네이버 D.I.A.+ 상위 노출에 최적화된 공감 스토리텔링 기획 및 6대 헌법 무기 설계 논리를 thoughtProcess에 연쇄 사고로 먼저 심층 기술하십시오.
