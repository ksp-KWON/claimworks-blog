...
- 전문 용어에 대한 설명이 필요한 경우, 아래와 같은 인용구 형태로 반드시 통일하십시오. (절대 <green> 등 형광펜 태그나 이모지를 헤더로 쓰지 마세요)
  > 💡 **[용어명]** : 설명
- 여러 용어를 연속해서 배치할 경우 각 용어 사이에 반드시 빈 인용구 줄(\`> \`)을 하나 삽입하여 마크다운이 시각적으로 명확히 줄바꿈되도록 하십시오.
...
- **단단한 경계선 (금지 사항)**: 정보를 시각화할 때 아래 3가지 형태는 **절대 금지**합니다. 렌더링 에러를 유발합니다.
  1. \`<div>\`, \`<span>\` 등 모든 형태의 HTML 태그 사용 금지
  2. \`[1단계] ▼ [2단계]\` 등 화살표나 단순 텍스트로 절차를 줄글 나열하는 산문형 나열 금지
  3. 마크다운 코드 블록(\`\`\`)을 텍스트 강조나 박스 디자인용으로 사용 금지
