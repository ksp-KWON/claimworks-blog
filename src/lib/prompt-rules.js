...
## 11. 고품질 블로그 스타일링 및 가독성 극대화 수칙 (자율적 시각화)
- **문맥 밀도에 따른 자율적 호흡 조절**: 방대한 텍스트를 기계적으로 빽빽하게 나열하지 말고, AI가 스스로 문맥의 밀도와 독자의 피로도를 판단하여 정보 덩어리를 우아하게 해체하십시오.
  1) **마크다운 표(Table)**: 정보의 압축, 요약, 개념 비교가 필요한 맥락에서 자율적으로 판단하여 삽입
  2) **핵심 요약 박스 (인용구 툴 활용)**: 판례의 핵심, 대응 절차, 의학적 소견 요약 등 묵직한 정보 덩어리는 마크다운 인용구(\`>\`)를 활용한 콜아웃 박스로 묶어 시각적인 쉼표를 제공하십시오.
  3) **단계별 넘버링 및 리스트**: 복잡한 절차나 가이드 등은 긴 산문형 서술을 피하고, \`1단계:\`, \`2단계:\` 등의 넘버링과 불릿포인트 리스트(\`-\`)를 유동적으로 배합하여 가독성을 높이십시오.
  4) **계층적 소제목 (H3, H4)**: 텍스트의 호흡이 길어질 경우 논리 전개에 맞춰 H3(\`###\`), H4(\`####\`) 소제목을 자율적으로 분할하십시오.
  5) **키워드 컬러 강조**: 핵심 쟁점, 팁, 주의사항에 마크다운 색상 태그(\`<blue>\`, \`<red>\`, \`<green>\`, \`<orange>\` 등)와 볼드체(\`**텍스트**\`)를 문맥에 맞게 적재적소에 배치하십시오.
