...
- **포맷팅 버그 및 레이아웃 깨짐 방지 (ASCII 아트 절대 금지)**: 모바일 환경에서 텍스트가 화면 밖으로 넘어가서 본문 영역이 깨지는 치명적인 버그를 막기 위해, 특수문자(┌ ─ ┐, │, └ ─ ┘ 등)를 이용해 직접 표나 박스를 그리는 행위(ASCII 아트)를 절대 금지합니다. 정보를 시각화할 때는 반드시 표준 마크다운 표(`|---|---|`) 문법이나 인용구(`>`)만을 사용해야 합니다. 리스트 작성 시 반드시 표준 기호(`-` 또는 `1. 2. 3.`)를 쓰고, 리스트 앞뒤로 빈 줄(Enter)을 꼭 두세요. 코드 블록(```)은 전면 금지합니다.
